...
If the toggle reads on but the panel still never dreams, the Screen saver section of Advanced device management prints the raw device state — enabled, whileCharging, whileDocked and the chosen dream. A missing dream component, or both trigger keys at zero, means there is no condition under which it would ever start.
