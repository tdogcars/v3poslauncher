...
Stage two and a half, turn the screen saver on. The launcher cannot switch it on by itself — the screen saver lives in Settings.Secure keys that are not on the Device Owner allowlist, and a Device Owner cannot grant itself WRITE_SECURE_SETTINGS because setPermissionGrantState only covers runtime permissions. Reading those keys needs no permission though, so the launcher always knows the real state: while the screen saver is off, the home screen shows a notice, and tapping it opens Android's screen saver page. Switch it on there, choose Clock, and the notice clears itself within a couple of seconds of returning. That is all a normal install needs.

Optionally, to skip even that: with the terminal still on USB run `adb shell pm grant com.flo.v3poslauncher android.permission.WRITE_SECURE_SETTINGS`. The launcher then configures the screen saver itself on every start and at boot, and the notice never appears. The grant survives reboots but not a reinstall.

A trap found on real MicroTouch hardware, and handled from v3.7.3 on. On a fresh M1-156IC the screen saver toggle can read on with every prerequisite correct — activate_on_sleep 1, the right timeout, stay-on released — and still show nothing, because screensaver_components is unset and the framework falls back to screensaver_default_component, which names `com.android.deskclock`. These are GMS builds where Google Clock (`com.google.android.deskclock`) replaces the AOSP clock, so the default resolves to no installed service. The panel then just goes black at the idle mark, which on a counter reads as dead hardware.

The launcher therefore never trusts the toggle on its own: it resolves the dream that would actually run against installed services, and treats "on but nothing resolves" as not active. That keeps the always-on display policy in place rather than releasing it into a black screen, and the home screen says "Screen saver is on, but no screen saver is chosen" with a tap through to the picker. Choosing Clock there fixes it permanently. With the WRITE_SECURE_SETTINGS grant the launcher repairs it silently instead, including replacing a stale component that does not resolve.

To confirm on a unit, drop the timeout, watch, and put it back: `adb shell settings put system screen_off_timeout 15000`, then `adb shell dumpsys dreams | grep mCurrentDreamName` while it is showing, then `adb shell settings put system screen_off_timeout 600000`. A healthy unit names the dream, e.g. ComponentInfo{com.google.android.deskclock/com.android.deskclock.Screensaver}. The Screen saver section of Advanced device management prints the same picture, including what will actually run.
